...
//MODIFIED: added readPartition clone function for display of feedback vertex sets

int graph_io::readVertexSet(graph_access & G, const std::string & filename) {
        std::string line;
        ...
        forall_nodes(G, node) {
                ...
                // in this line we find the block of Node node 
                G.setVertexsetIndex(node, (VertexSetID) atol(line.c_str()));

                // if(G.getSecondPartitionIndex(node) > max)
                //         max = G.getSecondPartitionIndex(node);
        } endfor
