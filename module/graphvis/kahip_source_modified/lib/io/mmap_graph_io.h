...
//MODIFIED: added library as in KaDraw's graph_io.cpp
#include <sstream>
...
//MODIFIED: added functions from KaDraw

void readCoordinates(graph_access & G, std::string filename) {
        std::string line;
...
void writeCoordinates(graph_access & G, std::string filename) {
        std::ofstream f(filename.c_str());
        forall_nodes(G, node) {
                f << G.getX(node) << " " << G.getY(node)  <<  std::endl;
        } endfor
        f.close();
}
