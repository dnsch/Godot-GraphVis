...
struct refinementNode {
    ...
    //MODIFIED: Added vertexsetIndex
    VertexSetID vertexsetIndex; 

    //MODIFIED: added coordtype
...
//MODIFIED: added definitions from KaDraw's graph_access.h

#define STRX(x) #x
#define STRINGIFY(x) STRX(x)
#define DIRECTIVESPECIAL(x) PRAGMA(omp parallel for reduction(x))
...
//MODIFIED: added definitions from KaDraw's graph_access.h
#define forall_nodes_parallel(G,n) { NodeID end = G.number_of_nodes(); _Pragma("omp parallel for schedule(dynamic, 1)") for(NodeID n = 0; n < end; ++n) {
#define forall_nodes_parallel_reduce(G,n,O,R) { NodeID end = G.number_of_nodes(); DIRECTIVESPECIAL(O:R) for(NodeID n = 0; n < end; ++n) {
//********************************************************
...
class graph_access {
        ...
        public:
                ...
                //MODIFIED: Added VertexsetIndex
                VertexSetID getVertexsetIndex(NodeID node);
                void setVertexsetIndex(NodeID node, VertexSetID id);

                //MODIFIED: added methods and member variables from KaDraw

                // ************************* KaDraw ********************************
                void setCoords(NodeID node, double x, double y); 
                double getX(NodeID node); 
                double getY(NodeID node); 
                // *****************************************************************
...
//MODIFIED: Added vertexsetIndex
inline VertexSetID graph_access::getVertexsetIndex(NodeID node) {
#ifdef NDEBUG
        return graphref->m_refinement_node_props[node].vertexsetIndex;
#else
        return graphref->m_refinement_node_props.at(node).vertexsetIndex;
#endif
}

inline void graph_access::setVertexsetIndex(NodeID node, VertexSetID id) {
#ifdef NDEBUG
        graphref->m_refinement_node_props[node].vertexsetIndex = id;
#else
        graphref->m_refinement_node_props.at(node).vertexsetIndex = id;
#endif
}

//MODIFIED: added methods from KaDraw
// ************************* KaDraw ********************************
inline double graph_access::getX(NodeID node) {
#ifdef NDEBUG
        return graphref->m_refinement_node_props[node].x;
#else
        return graphref->m_refinement_node_props.at(node).x;
#endif
}

inline double graph_access::getY(NodeID node) {
#ifdef NDEBUG
        return graphref->m_refinement_node_props[node].y;
#else
        return graphref->m_refinement_node_props.at(node).y;
#endif
}

inline void graph_access::setCoords(NodeID node, double x, double y) {
#ifdef NDEBUG
        graphref->m_refinement_node_props[node].x = x;
        graphref->m_refinement_node_props[node].y = y;
#else
        graphref->m_refinement_node_props.at(node).x = x;
        graphref->m_refinement_node_props.at(node).y = y;
#endif
}
