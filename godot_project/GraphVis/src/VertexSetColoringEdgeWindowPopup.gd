...
func _ready() -> void:
	GlobalSettings.connect("vertexsetid_list_changed", self, "_on_vertexsetid_list_changed")

func _on_vertexsetid_list_changed():
	...
	checkbox_button_arr = []
	if (GlobalVariables.vertex_sets_color_init):
		for partition_id in range(2):
			...
			GlobalVariables.vertex_sets_with_colors[partition_id] = Color(1,1,1,1)
			color_picker_button_arr.append(new_color_picker)
			checkbox_button_arr.append(new_visible_check_box)
			current_config[partition_id ]= GlobalVariables.vertex_sets_with_colors[partition_id]
		current_partitions_edge_color = Color(0,0,0,1)
		GlobalVariables.vertex_sets_color_init = false
	else:
		for partition_id in range(2):
			var new_panel = Panel.new()
			grid_container.add_child(new_panel)
			...
			below_node = new_panel
			var new_color_picker = ColorPickerButton.new()
			new_color_picker.color = GlobalVariables.vertex_sets_with_colors[partition_id]
			new_color_picker.set_default_cursor_shape(2)
			new_color_picker.set_anchors_preset(15)
			new_color_picker.set_h_size_flags(3)
			grid_container.add_child_below_node(below_node, new_color_picker)
			...
			checkbox_button_arr.append(new_visible_check_box)
			current_config[partition_id ]= GlobalVariables.vertex_sets_with_colors[partition_id]
		current_partitions_edge_color = GlobalVariables.vertex_sets_edge_color
...
func _on_ApplyButton_pressed() -> void:
	for partition_id in range(2):
		GlobalVariables.vertex_sets_with_colors[partition_id] = current_config[partition_id]
	partitions_edge_color_picker.color = current_partitions_edge_color
	GlobalVariables.vertex_sets_edge_color = current_partitions_edge_color
...
func _on_DontApplyButton_pressed() -> void:
	var part_id = 0
	for color_picker_button in color_picker_button_arr:
		color_picker_button.color = GlobalVariables.vertex_sets_with_colors[int(part_id)]
		part_id += 1
	partitions_edge_color_picker = GlobalVariables.vertex_sets_edge_color
	visible = false
